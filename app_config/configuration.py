...
import os
import sys
from app_entity import DataIngestionConfig, TrainingPipelineConfig, DataValidationConfig, DataTransformationConfig, ModelTrainerConfig,ModelEvaluationConfig, ModelPusherConfig
...
DATA_INGESTION_MERGE_CSV_KEY = "merged_csv_dir"
DATA_INGESTION_SPLIT_KEY = "splited_data_dir"
DATA_INGESTION_GOOD_DIR_KEY = "ingested_good_data_dir"
DATA_INGESTION_BAD_DIR_KEY = "ingested_bad_data_dir"
...
CLUSTERING_MODEL_DIR_KEY = "clustering_model_dir"
...
class AppConfiguration:
    ...
    def get_data_ingestion_config(self) -> DataIngestionConfig:
        try:
            ...
            raw_data_dir = os.path.join(artifact_dir, data_ingestion_config[DATA_INGESTION_RAW_DATA_DIR_KEY])
            tgz_download_dir = os.path.join(artifact_dir, data_ingestion_config[DATA_INGESTION_TGZ_DOWNLOAD_DIR_KEY])
            ingested_dir_name = os.path.join(artifact_dir,data_ingestion_config[DATA_INGESTION_DIR_NAME_KEY])
            ingested_good_data_dir = os.path.join(ingested_dir_name,data_ingestion_config[DATA_INGESTION_GOOD_DIR_KEY])
            ingested_bad_data_dir = os.path.join(ingested_dir_name,data_ingestion_config[DATA_INGESTION_BAD_DIR_KEY])
            merged_csv_dir = os.path.join(artifact_dir,data_ingestion_config[DATA_INGESTION_MERGE_CSV_KEY])
            splited_data_dir = os.path.join(artifact_dir,data_ingestion_config[DATA_INGESTION_SPLIT_KEY])
            ingested_train_dir = os.path.join(splited_data_dir,data_ingestion_config[DATA_INGESTION_TRAIN_DIR_KEY])
            ingested_test_dir = os.path.join(splited_data_dir,data_ingestion_config[DATA_INGESTION_TEST_DIR_KEY])

            response = DataIngestionConfig(dataset_download_url=data_ingestion_config[DATA_INGESTION_DOWNLOAD_URL_KEY],
                                           raw_data_dir=raw_data_dir,
                                           tgz_download_dir=tgz_download_dir,
                                           ingested_good_data_dir=ingested_good_data_dir,
                                           ingested_bad_data_dir=ingested_bad_data_dir,
    ...
    def get_data_transformation_config(self) -> DataTransformationConfig:
        try:
            ...
            transformed_dir = os.path.join(artifact_dir, data_transormation_config[DATA_TRANSFORMATION_DIR_NAME_KEY])
            transformed_train_dir = os.path.join(transformed_dir, data_transormation_config[DATA_TRANSFORMATION_TRAIN_DIR_NAME_KEY])
            transformed_test_dir = os.path.join(transformed_dir, data_transormation_config[DATA_TRANSFORMATION_TEST_DIR_NAME_KEY])
            preprocessing_dir = os.path.join(artifact_dir, data_transormation_config[DATA_TRANSFORMATION_PREPROCESSING_DIR_KEY])
            preprocessed_file_name = data_transormation_config[DATA_TRANSFORMATION_PREPROCESSED_FILE_NAME_KEY]
            preprocessed_object_file_path = os.path.join(preprocessing_dir, preprocessed_file_name)

            response = DataTransformationConfig(transformed_train_dir=transformed_train_dir,
    ...
    def get_model_trainer_config(self) -> ModelTrainerConfig:
        try:
            ...
            clustering_model_dir =  os.path.join(artifact_dir, model_trainer_config[CLUSTERING_MODEL_DIR_KEY])
